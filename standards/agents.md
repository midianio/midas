...
- **`midas sync` writes a version-stamped managed block** into each repo's `AGENTS.md` (required by
  AGT-0001). If `CLAUDE.md` already exists, the same block is refreshed there — the file is not
  created, and a missing or stale copy is not check-tier drift. Content and wording are generated
  by `cmd/sync.rs::managed_block` — this is a live example, not a frozen copy; if the two ever
  disagree, the source is truth (run `midas sync` on this repo to see the exact current text):
