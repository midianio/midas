...
Each repo already has its own agent config (every repo has its own `AGENTS.md`, `.cursor`). The
standard injects shared guidance **without clobbering it**:

- **`midas sync` writes a version-stamped managed block** into each repo's `AGENTS.md` (required by
  AGT-0001). Content and wording are generated by `cmd/sync.rs::managed_block` — this is a live
