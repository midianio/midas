...
- **OPS-0005 [check]** — `midflow doctor` is the readiness gate: `git`/`gh`/`go`/`pscale` on PATH,
...
- **OPS-0006 [check]** — midflow **owns** the connection string. `start`/`db` write
  `MYSQL_DATABASE_URL` into `app/api/.env.local` between `# >>> midflow >>>` / `# <<< midflow <<<`
  markers (`internal/flow/env.go:14-64`), scoped to the active branch. Never hand-edit that block —
  it's rewritten on every `start` and stripped on `db end`.
- **OPS-0006 [check]** — Env load order is first-wins and never overrides a real process var:
  `ENV_FILE` → crate `.env` (committed dev creds) → `.env.local` (the midflow tunnel block)
...
## Release & branch flow — `midflow`

The flow is a Go CLI, not a wiki page: `start` → commit → `sync` → `pr` → squash-merge → (promote)
...
- **OPS-0007 [check]** — Branches are `<type>/<slug>` cut off `origin/dev`, type ∈
  ...
  (`start.go:96-98`). `midas` should not reimplement this — shell out to / vendor midflow.
- **OPS-0001 [review]** — Go through midflow for branch/PR/sync/tag; don't hand-roll the git dance.
...
- **OPS-0001 [review]** — **Hotfix** path is `midflow hotfix <slug>` = `start` locked to `fix/` off
  `dev`, with a "add a regression test" reminder (`internal/cmd/hotfix.go:9-15`). For a fire so urgent
...
- **OPS-0008 [check]** — Migrations are **forward-only**, numbered `NNN_short_name.sql` in
  `db/migrations/` (currently `001`–`018`), applied in lexical order; the runner records each file in a
  `_migrations` ledger and re-runs are no-ops (`db/cmd/migrate/main.go:9-11`; `db/README.md:88-97`).
  One DDL set per file, **no `BEGIN`/`COMMIT`** (Vitess forbids DDL-in-txn) — a mid-file failure leaves
  partial state, so **fix forward, never edit an applied file** (`db/README.md:93-95,126-135`).
  Mirrors **BE-0007**.
- **OPS-0009 [review]** — Schema reaches an integration/prod branch only through a **PlanetScale deploy
  request reviewed in the PS UI** — never run `cmd/migrate` against it (`db/README.md:47-48`). When a
  PR touches `db/migrations/**`, CI ensures the pscale branch, applies migrations with a 1-hour
  temp password, and opens/refreshes the DR (`.github/workflows/db-deploy-request.yml`). **DR approval
  is deliberately *not* cascaded from GitHub PR review** — it's a separate manual click
  (`db-deploy-request.yml:13-14`). On merge, CI deploys the DR and deletes the pscale branch
  (`db-deploy.yml:71-97`).
- **OPS-0009 [check]** — Clean up paired pscale branches: `midflow db end [--force]`. `--force` deletes
  the branch but **refuses when the flow was git-only** (target is shared `dev`), and a hardcoded
...
- **OPS-0004 [review]** — Destructive prod data ops (TRUNCATE/DELETE/DROP, manual deploys) are handed
  to a human with the exact commands — tooling and agents never execute them. The migrate binary is
  dev/preview-only by construction (`db/cmd/migrate/main.go:8-9`); prod is DR-gated (OPS-0009).
...
| OPS-0006 | Local dev = pscale proxy `:3309` + dotenv chain (`ENV_FILE`→`.env`→`.env.local`); midflow owns the `.env.local` tunnel block — don't hand-edit. | check | hard |
