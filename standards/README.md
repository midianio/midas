...
- **Tier** — how it's enforced: `check` (a defined mechanical check, run by `midas check`) or
  `review` (semantic → human/agent review; `SPEC.md §8`). A rule is `check`-tier **only if the
  registry carries a real check spec for it** — aspirationally-mechanical rules stay `review` until
  their check exists, so the gate never silently skips a rule it claims to enforce.
- **Escape** — how it may be deviated from: `hard` (no deviation — a `[deviations]` entry against a
  `hard` rule is itself an error), `ledgered` (allowed if recorded in `midas.toml [deviations]`),
...
| L6 · Documentation | [`docs.md`](./docs.md) (adoption: [`playbooks/adopt-docs.md`](./playbooks/adopt-docs.md)) | written |
...
| ID | Rule | Tier | Escape |
| --- | --- | --- | --- |
| STK-0001 | Backend = Rust (axum 0.8 · tokio · sqlx 0.8 · utoipa). | review | ledgered |
| STK-0002 | Frontend = SvelteKit 2 + Svelte 5 runes; package manager = Bun. | review | ledgered |
| STK-0003 | Native/PWA = Capacitor 8 via static-adapter switch. | review | ledgered (web-only) |
| STK-0004 | Primary DB = PlanetScale/Vitess (MySQL); migrations forward-only. | review | ledgered |
...
| BE-0002 | One wire envelope `{data,code,timestamp,count}`; helpers `response::ok/ok_list`. | review | hard |
| BE-0003 | Errors via one `AppError` enum + `IntoResponse`; never leak internals; details logged. | review | hard |
| BE-0004 | Auth via the `RequireAuth` extractor; no fallback identity (any failure → 401). | review | hard |
| BE-0005 | Authz via the central `access::require` seam; never scatter `WHERE user_id = ?`. | review | hard |
| BE-0006 | Feature gating via `RequirePlan` + `usage::guard`/`Pass`; no hand-rolled per-handler checks. | review | hard |
| BE-0007 | Forward-only migrations; never edit an applied migration in place. | review | hard |
| BE-0008 | camelCase on the wire, snake_case in code (`#[serde(rename_all="camelCase")]`). | review | hard |
...
| BE-0018 | Prefer compile-checked `query!`/`query_as!`; commit the `.sqlx` offline cache (artifact-commit overlaps OPS-0003). | review | ledgered |
...
| ID | Rule | Tier | Escape |
| --- | --- | --- | --- |
| CLI-0001 | Agent-runnable: non-interactive by default; every prompt has a flag path; no-TTY never prompts. | review | hard |
| CLI-0002 | Dual output: `--json` with a stable, documented schema on every data-returning command. | review | hard |
| CLI-0003 | stdout = data, stderr = logs/progress/prompts. | review | hard |
| CLI-0004 | Typed, documented exit codes (`0`/`1`/`2`/`3`/`4`). | review | hard |
| CLI-0005 | Built on one internal CLI contract kernel (`cli/src/core/`, clap derive); not re-implemented per command. | review | hard |
...
| CLI-0008 | Snapshot-tested surface (`assert_cmd`): output, `--json` schema, exit codes. | review | hard |
...
| OPS-0008 | Migrations forward-only, numbered `NNN_`, one DDL set/file, fix-forward. *(cross-ref BE-0007 — same rule, process view)* | review | hard |
